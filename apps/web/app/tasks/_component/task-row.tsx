...
export default function TaskRow({
  ...
  return (
    <>
      ...
      >
        <div onClick={() => onToggleStatus(task)} className="cursor-pointer hover:opacity-30 hover:scale-90 duration-300">
          {StatusIconRender(task.status)}
        </div>

        <div onClick={() => onOpenChange(true)} className="flex-1 cursor-pointer">
          <p className={cn(task.status === TaskStatus.DONE ? 'line-through text-muted-foreground' : '', 'select-none')}>
            {task.taskName}
          </p>
